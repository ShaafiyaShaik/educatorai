#!/usr/bin/env python3
"""
Test sending individual message to Jennifer WITHOUT attendance to isolate the error
"""
import requests
import json

def test_individual_message_simple():
    """Test sending individual message to Jennifer with simple template"""
    
    print("📧 Testing Individual Message to Jennifer (Simple Template)")
    print("=" * 60)
    ...
    try:
        ...
        # Send individual message to Jennifer WITH SIMPLE TEMPLATE (no attendance)
        message_data = {
            ...
            if result.get('performance_data'):
                for perf in result['performance_data']:
                    if 'jennifer' in perf['student_name'].lower():
                        ...
                        print(f"   Has attendance field: {'attendance_percentage' in perf}")
                        break
        else:
...
if __name__ == "__main__":
    test_individual_message_simple()